...
When you click the Deploy to Azure button, Azure Portal will display a security prompt asking "Do you trust the authors code?" This is **normal and expected** for GitHub-hosted ARM templates.

**Why this appears**: The deployment UI uses Azure's API to dynamically query your subscription for existing resource groups, Event Hub namespaces, and storage accounts. Azure shows this security prompt whenever a template makes ARM API calls on your behalf.

**It's safe to proceed**: Click **"Yes, I trust the authors"** to continue with the deployment. This template is:
- ✅ Open-source and auditable in the [Edge Delta Azure Resources repository](https://github.com/edgedelta/edgedelta-azure-resources)
- ✅ Published from the official Edge Delta organization
- ✅ Uses standard Azure ARM APIs to query existing resources
- ✅ Makes no external service calls or custom scripts

This trust prompt is standard for community and vendor-provided templates hosted on GitHub, including Microsoft's own Azure Quickstart Templates when they use dynamic resource queries.
